...
export function IndexingPanel() {
  ...
  // single check on mount — no continuous polling until user starts indexing
  useEffect(() => {
  ...
  const startIndexing = async () => {
    ...
    try {
      await photosApi.startIndexing(folder.trim())
      startPolling()
    } catch (e: any) {
      alert(e.response?.data || 'Failed to start indexing')
    }
  }
  ...
  return (
    <div className="bg-slate-800 rounded-xl p-6 mb-6">
      <h2 className="text-lg font-semibold mb-4 text-white">סריקת תיקיית תמונות</h2>

      {!backendUp && (
        <p className="text-red-400 text-sm mb-3">⚠ הבקאנד לא פעיל — הרץ את start.bat</p>
      )}
      ...
      <div className="flex gap-3 mb-4">
        ...
        >
          {status?.isIndexing ? 'סורק...' : 'התחל סריקה'}
        </button>
      </div>
      ...
      {status?.isIndexing && (
        <div>
          <div className="flex justify-between text-sm text-slate-400 mb-1">
            ...
            <span className="shrink-0 mr-4">{status.processed.toLocaleString()} / {status.total.toLocaleString()}</span>
          </div>
      ...
      {status && !status.isIndexing && status.processed > 0 && (
        <p className="text-green-400 text-sm">
          ✓ נסרקו {status.processed.toLocaleString()} תמונות בהצלחה
        </p>
      )}
    </div>
  )
