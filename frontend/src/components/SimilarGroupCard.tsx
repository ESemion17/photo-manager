...
export function SimilarGroupCard({ group, onResolved }: Props) {
  const [selected, setSelected] = useState<Set<number>>(
    new Set(group.photos.map(p => p.photoId))
  )
  ...
  const resolve = async () => {
    setLoading(true)
    try {
      await similarityApi.resolveGroup(group.id, [...selected])
      onResolved()
    } catch (e) {
    ...
    <div className="bg-slate-800 rounded-xl p-4 mb-4">
      <div className="flex justify-between items-center mb-3">
        <span className="text-sm text-slate-400">
          דמיון: <span className="text-yellow-400 font-bold">{group.similarityScore.toFixed(1)}</span>
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={resolve}
            disabled={loading}
            className="text-xs bg-blue-600 hover:bg-blue-700 disabled:bg-slate-600 text-white px-4 py-1 rounded-lg transition-colors"
          >
            {loading ? '...' : 'אשר'}
          </button>
        </div>
          ...
          return (
            <div
              ...
              className={`relative cursor-pointer rounded-lg overflow-hidden border-2 transition-all ${
                isKept ? 'border-green-500' : 'border-red-500 opacity-50'
              }`}
            >
              <img
              ...
              <div className={`absolute top-2 right-2 rounded-full w-6 h-6 flex items-center justify-center text-xs font-bold ${
                isKept ? 'bg-green-500 text-white' : 'bg-red-500 text-white'
              }`}>
                {isKept ? '✓' : '✕'}
              </div>
      ...
      {expanded !== null && (
        ...
        >
          <img
            src={`http://localhost:5000/api/photos/${expanded}/image`}
            alt=""
