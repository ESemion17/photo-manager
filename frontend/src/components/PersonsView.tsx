...
export function PersonsView() {
  ...
  const load = async () => { setPersons(await personsApi.list()) }
  useEffect(() => { load() }, [])

  const addPerson = async () => {
    if (!newName.trim()) return
    await personsApi.create(newName.trim()); setNewName(''); load()
  }

  const saveName = async (id: number) => {
    if (!editName.trim()) return
    await personsApi.rename(id, editName.trim()); setEditId(null); load()
  }

  return (
    <div>
      <div className="bg-slate-800 rounded-xl p-6 mb-6">
        <h2 className="text-lg font-semibold text-white mb-4">{t('persons.title')}</h2>
        <div className="flex gap-3">
          ...
          />
          <button onClick={addPerson} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg font-medium">
            {t('persons.addButton')}
          </button>
        </div>
      </div>
            ...
            {editId === person.id ? (
              ...
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-slate-400">{t('persons.photoCount', { count: person.photoCount })}</p>
                </div>
                <button onClick={() => { setEditId(person.id); setEditName(person.name) }} className="text-slate-400 hover:text-slate-300 text-sm">
                  {t('persons.editButton')}
                </button>
              </div>
            )}
          </div>
      ...
      {persons.length === 0 && (
        <p className="text-center text-slate-500 py-12">{t('persons.empty')}</p>
      )}
    </div>
  )
}
